...
export function useOthelloGame(
  ...
  aiModelUrl?: string,
  initialState?: {
    bitboardBlack: bigint;
    bitboardWhite: bigint;
  ...
  const updateGameState = useCallback(
    (g: WasmGame, aiThinking: boolean = false) => {
      setState({
        board: g.board(),
  ...
  const initialiseGame = useCallback(async () => {
    try {
      ...
      if (initialState) {
        g = WasmGame.new_from_state(
          initialState.bitboardBlack,
          initialState.bitboardWhite,
          initialState.currentTurn == "black" ? 1 : 2,
          gameType,
        );
      } else {
        // GameType 2 = PlayerVsModel
        g = new WasmGame(gameType);
      }

      // Load ONNX evaluator if not already loaded
      if (!evaluatorRef.current) {
        evaluatorRef.current = await createBatchEvaluator(aiModelUrl);
      }
  ...
  }, [initialState, updateGameState, gameType, aiModelUrl]);
